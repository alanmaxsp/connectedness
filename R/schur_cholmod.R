...
.cd_contrast_mu_schur_cholmod_lowmem_R <- function(Kinv,
                                                   id_rec,
                                                   Xsp,
                                                   mu_animal,
                                                   target,
  ...
  if (!.is_sparse_matrix(Kinv)) {
    stop("'Kinv' must be a sparse Matrix object for schur_solver = 'cholmod_lowmem'.")
  }
  if (sigma2a <= 0) stop("'sigma2a' must be > 0.")
  if (sigma2e < 0) stop("'sigma2e' must be >= 0.")
  ...
  x_block_size <- as.integer(x_block_size)
  if (length(x_block_size) != 1L || is.na(x_block_size) || x_block_size < 1L) {
    stop("'x_block_size' must be a positive integer.")
  }
  ...
  if (verbose) {
    message(sprintf(
      "Schur-CHOLMOD-lowmem: Cuu dim = %d x %d, nnz stored = %d",
      nrow(Cuu_sym), ncol(Cuu_sym), Matrix::nnzero(Cuu_sym)
    ))
    message(sprintf(
      "Schur-CHOLMOD-lowmem: Kinv dim = %d x %d, nnz stored = %d",
      nrow(Kinv_sym), ncol(Kinv_sym), Matrix::nnzero(Kinv_sym)
    ))
  }

  if (verbose) message("Schur-CHOLMOD-lowmem: factorizing Cuu...")
  fac_Cuu <- .cd_cholesky(Cuu_sym, "Cuu")

  if (verbose) message("Schur-CHOLMOD-lowmem: factorizing Kinv...")
  fac_Kinv <- .cd_cholesky(Kinv_sym, "Kinv")

  if (verbose) message("Schur-CHOLMOD-lowmem: building XtX and Z'X...")
  XtX <- crossprod(Xsp)
  Z <- Matrix::sparseMatrix(
    i = seq_along(id_rec),
    j = id_rec,
  ...
  if (verbose) {
    message(sprintf(
      paste0(
        "Schur-CHOLMOD-lowmem: building fixed-effect Schur complement by X blocks; ",
        "X has %d columns, x_block_size = %d"
      ),
      p, x_block_size
    ))
  }
  ...
  S <- as.matrix(XtX)
  for (j0 in seq(1L, p, by = x_block_size)) {
    j1 <- min(j0 + x_block_size - 1L, p)
    jj <- j0:j1

    if (verbose) {
      message(sprintf("Schur-CHOLMOD-lowmem: processing X block %d-%d / %d", j0, j1, p))
    }

    ZtX_blk <- ZtX[, jj, drop = FALSE]
    W_blk <- as.matrix(.cd_solve_cholesky(
      fac_Cuu,
      as.matrix(ZtX_blk),
      sprintf("W block %d-%d = Cuu^{-1}Z'X_block", j0, j1)
    ))

    S[, jj] <- S[, jj] - as.matrix(crossprod(ZtX, W_blk))
  ...
  chol_S <- tryCatch(
    ...
    error = function(e) {
      stop(sprintf(
        "Schur-CHOLMOD-lowmem: factorization of fixed-effect Schur complement failed. Check collinearity in fixed effects: %s",
        conditionMessage(e)
      ), call. = FALSE)
    }
  )
  ...
  for (j0 in seq(1L, U, by = block_size)) {
    ...
    if (verbose) {
      message(sprintf("Schur-CHOLMOD-lowmem: processing MU block %d-%d / %d", j0, j1, U))
    }
    ...
    rhs_wb <- ZtX %*% b_blk
    Wb_blk <- as.matrix(.cd_solve_cholesky(
      fac_Cuu,
      as.matrix(rhs_wb),
      "Wb = Cuu^{-1}Z'Xb"
    ))
    ...
    rm(B_blk, R_blk, rhs_b, b_blk, rhs_wb, Wb_blk, U_blk, YK_blk)
    gc(FALSE)
  }
